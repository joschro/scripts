...
ping -c1 -w1 192.168.178.1 >/dev/null && {
  ...
  # Elgato Keylights
  ping -c1 -w1 $elgato_ip >/dev/null 2>&1 && elgato-key-light-an.sh >/dev/null && sleep 1
  # Tasmota busy lights
...
for busylight_ip in $(~/bin/ip-of-mac.sh "$MACs"); do 
  # Tasmota busy lights
  ping -c1 -w1 $busylight_ip >/dev/null 2>&1 && curl --silent http://$busylight_ip/cm?cmnd=Power%20ON >/dev/null
done

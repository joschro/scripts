...
test $# -lt 1 && {
	...
	a=$(echo "scale=4; ($percent8h - $percent1h) / (7 * 3600)" | bc -l)#; echo "a=$a"
	b=$(echo "scale=4; $percent8h - $a * 8 * 3600" | bc -l)#; echo "b=$b"
	percentGoal="$(echo "scale=0; (( $a * $SUN_SECS + $b ) + 0.5 ) / 1" | bc)"#; echo "percentGoal=$percentGoal"
...
at "\"$(sh ~/bin/tibber-fetch-prices.sh $myConfigPath $numberOfQuarterlyHours)\" <<< \"sh ~/bin/sonnenbatterie.sh ~/Projekte/github/private --until $percentGoal laden\""
